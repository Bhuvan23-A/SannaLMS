        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div>
            ...
            <select required className="input-field" value={subjectId} onChange={e => { setSubjectId(e.target.value); setSectionId(''); }}>
              ...
              {subjects.length === 0 ? <option value="" disabled>No subjects yet — add them under Subjects first</option>
                : subjects.map((s: any) => <option key={s.id} value={s.id}>{s.code} — {s.name}{s.credits ? ` (${s.credits} cr)` : ''}</option>)}
            </select>
            {selectedSubject && (
